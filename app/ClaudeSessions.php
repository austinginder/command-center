...
class ClaudeSessions {
	...
	public static function toolCategory( string $tool ): string {
		return match ( $tool ) {
			...
			'Skill'                                    => 'skill',
			'TodoWrite', 'NotebookEdit', 'LSP',
			'UpdateGoal', 'AskUserQuestion',
			'EnterPlanMode', 'ExitPlanMode',
			'ImageGen', 'ImageEdit', 'ImageToVideo'    => 'misc',
			default                                    => 'other',
		};
	}
	...
	public static function describeToolCall( string $tool, array $input ): string {
		return match ( $tool ) {
			...
			'ToolSearch'     => $input['query'] ?? $input['tool_name'] ?? '',
			...
			'UpdateGoal'     => mb_substr( (string) ( $input['message'] ?? $input['description'] ?? '' ), 0, 120 ),
			'AskUserQuestion'=> mb_substr( (string) ( $input['question'] ?? $input['header'] ?? $input['prompt'] ?? '' ), 0, 120 ),
			...
			'EnterWorktree'  => '',
			'ImageGen'       => mb_substr( (string) ( $input['prompt'] ?? '' ), 0, 120 ),
			'ImageEdit'      => mb_substr( (string) ( $input['prompt'] ?? '' ), 0, 120 ),
			'ImageToVideo'   => mb_substr( (string) ( $input['prompt'] ?? '' ), 0, 120 ),
